...
FOREIGN_OVERRIDES = {
    ...
    "55703": "도미니카",    # 루이스 리베라토 (career 'Esperanza Milena Martinez (고)' — 국가토큰 없음)
}
...
def classify_player(player_id, career, season=2026):
    ...
    # 1.5) career 가 국가명으로 시작해도 한국 아마추어 파이프라인(초등학교 토큰)이 있으면
    #      어린 시절 해외 수학 후 국내 육성된 한국 선수다
    #      (예: 유민상 '미국 윌리캐넌초-잠신중-서울고-연세대').
    #      KBO 외국인 선수 career 는 초등학교를 표기하지 않으므로 '한글+초+구분자' 는 국내 신호.
    #      (단순 '초' 포함이 아니라 학교 토큰 형태로 한정해 외국 지명·음차의 오탐을 막는다.)
    if nat and re.search(r"[가-힣]초[-\s/]", career or ""):
        return ("대한민국", 0, "국내")
