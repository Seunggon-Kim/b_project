...
라이브 응답은 구조만 봅니다
---------------------------
`/standings`, `/schedule` 은 네이버에서 실시간으로 받아 옵니다. 정답지를 뜬
순간과 이식본을 호출한 순간의 값이 다를 수밖에 없어, 값을 그대로 비교하면 늘
불일치로 잡힙니다. 이런 응답은 값 대신 **구조**(키 이름과 타입)를 비교합니다.
숫자가 문자열로 바뀌는 이식 사고는 이 방식으로도 잡힙니다.

`/players/{id}/news` 도 라이브였지만 라우트를 없앴습니다.
"""
import argparse
import json
import math
...
# 옛 정답지에는 선수 뉴스가 들어 있습니다. 라우트를 없앴으므로 새로
# 뜨는 정답지에는 없지만, 남아 있는 파일을 비교할 때 값 비교로 잡히지
# 않도록 이 목록은 그대로 둡니다.
LIVE_SUFFIXES = ("_news",)
