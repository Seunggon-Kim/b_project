...
### 함정: 파생표를 먼저 재계산하면 2026 이 사라집니다

`wrc_plus_comparison` 과 `weighted_pf_by_batter_season` 에는 2026 값이
있습니다. EC2 가 계산해 둔 것을 덤프에서 복원했습니다. 그런데 원천
`play_by_play` 에 2026 이 없습니다.

**이 상태로 `park_factors/run_pipeline.sh` 를 돌리면 지금 있는 2026 값이
날아갑니다.** 스크립트가 `DELETE` 후 다시 넣기 때문입니다. Task 5 의
순서를 반드시 지키십시오. 2026 pbp 를 채운 뒤에 재계산합니다.
...
**2026 pbp 를 채운 뒤에 하십시오.** 파생표에 남아 있는 2026 값은 EC2 가
계산한 것이고, 원천이 빈 상태로 재계산하면 그 값이 지워집니다.
